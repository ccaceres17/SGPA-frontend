...
function buildFullPayload(user, isActive) {
  const payload = {
    ...
    id_role: Number(user.id_role || ROLE_IDS.student),
...
export async function load({ fetch }) {
  try {
    ...
    const students = users
      .filter((user) => Number(user.id_role) === ROLE_IDS.student)
      .map((user) => ({
        ...user,
    ...
    return {
      users: [],
      totalUsers: 0,
      error: error.message || 'Could not load students.'
    };
  }
}
...
export const actions = {
  toggleStatus: async ({ request, fetch }) => {
    ...
    return {
      ...
      message: isActive ? 'Student enabled successfully.' : 'Student disabled successfully.'
    };
  },
